...
export function initScrollProtection() {
  ...
  const handleScroll = () => {
    ...
    if (isUserScrolling && currentPosition < lastScrollPosition - 100 && currentPosition < 100) {
      // Восстанавливаем позицию
      console.warn('Scroll jump detected, restoring position');
  ...
  window.scrollTo = function(...args) {
    ...
    if (args.length > 0 && !isInitialLoad) {
      ...
      if (targetY === 0 || (typeof targetY === 'number' && targetY < 50)) {
        const currentPosition = window.pageYOffset || document.documentElement.scrollTop || 0;
        // Если пользователь скроллил и позиция была больше 100px, не сбрасываем
        ...
        if (currentPosition > 100 && Date.now() - (lastScrollTime || 0) < 500) {
